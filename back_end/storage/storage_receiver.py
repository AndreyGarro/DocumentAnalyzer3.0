...
class storage_receiver:
    def __init__(self, host='localhost'):
        ...
        self.channel = None
        self.queues = ['storage_sas', 'storage_new_user', "storage_download"]
        self.routing_keys = ['sas_rk', 'new_user_rk', "download_rk"]
        self.functions = {
            'storage_sas': self.get_sas,
            'storage_new_user': self.new_user,
            'storage_download': self.download_file
        }
    ...
    def get_sas(self, ch, method, props, body):
        ...
        response_ = "[{\"sas\": \"" + storage.get_sas(int(id)) + "\"}]"
        print(response_)
    ...
    def new_user(self, ch, method, props, body):
        ...
        response_ = storage.create_container(body)
        ch.basic_publish(exchange='',
                         routing_key=props.reply_to,
                         properties=pika.BasicProperties(correlation_id= props.correlation_id),
                         body=response_)
    ...
    def download_file(self, ch, method, props, body):
        body = str(body.decode("utf-8"))
        user = json.loads(body)['user']
        filename = json.loads(body)['file']

        response_ = storage.download(user, filename)
